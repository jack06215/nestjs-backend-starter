...
export class Migration20240128144001 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "posts" ("id" serial primary key, "title" varchar(255) not null, "content" varchar(255) not null);');

    this.addSql('create table "users" ("id" uuid not null, "name" varchar(48) not null, "email" varchar(128) not null, "is_ready_to_play" boolean not null default true, "created_at" timestamptz not null, "updated_at" timestamptz not null, constraint "users_pkey" primary key ("id"));');
